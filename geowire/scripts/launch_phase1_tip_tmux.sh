...
MANIFEST="${MANIFEST:?set MANIFEST to a Georoute JSONL manifest}"
CACHE_ROOT="${CACHE_ROOT:?set CACHE_ROOT to a Georoute cache root}"
...
TRAIN_CMD="'${PYTHON_BIN}' scripts/train_tip.py --manifest '${MANIFEST}' --cache-root '${CACHE_ROOT}' --output '${OUTPUT_DIR}' --steps '${STEPS}' --save-every '${SAVE_EVERY}' --device '${DEVICE}' --tip-feature-mode '${TIP_FEATURE_MODE}' --qwen-checkpoint '${QWEN_CHECKPOINT}' --dtype '${DTYPE}' --train-micro-batch-size-per-gpu '${TRAIN_MICRO_BATCH_SIZE_PER_GPU}'"
if [[ "${NPROC_PER_NODE}" != "1" ]]; then
  TRAIN_CMD="'${PYTHON_BIN}' -m torch.distributed.run --standalone --nproc_per_node='${NPROC_PER_NODE}' scripts/train_tip.py --manifest '${MANIFEST}' --cache-root '${CACHE_ROOT}' --output '${OUTPUT_DIR}' --steps '${STEPS}' --save-every '${SAVE_EVERY}' --device '${DEVICE}' --tip-feature-mode '${TIP_FEATURE_MODE}' --qwen-checkpoint '${QWEN_CHECKPOINT}' --dtype '${DTYPE}' --train-micro-batch-size-per-gpu '${TRAIN_MICRO_BATCH_SIZE_PER_GPU}'"
fi
